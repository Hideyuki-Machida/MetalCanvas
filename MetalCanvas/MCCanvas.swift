    ...
    public func fill(commandBuffer: inout MTLCommandBuffer, color: MCColor) throws {
        ...
        case .perspective:
            object = try MCPrimitive.Rectangle.init(
                position: SIMD2<Float>.init(x: -1.0, y: -1.0),
                w: 2.0,
                h: 2.0,
                color: color
            )
        case .center:
            object = try MCPrimitive.Rectangle.init(
                position: SIMD2<Float>.init(x: -Float(self.drawInfo.renderSize.width / 2.0), y: -Float(self.drawInfo.renderSize.height / 2.0)),
        ...
        case .topLeft, .bottomLeft:
            object = try MCPrimitive.Rectangle.init(
                position: SIMD2<Float>.init(x: 0.0, y: 0.0),
                w: Float(self.drawInfo.renderSize.width),
                h: Float(self.drawInfo.renderSize.height),
                color: color
            )
